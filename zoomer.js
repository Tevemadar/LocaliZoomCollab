...
//        while((cutw>=canvaswidth*2)&&(cuth>=canvasheight*2)&&(level<maxlevel)){
        let level=cfg.MaxLevel;
        while((cutw>=canvaswidth*2)&&(cuth>=canvasheight*2)&&(level>0)){
        ...
        for(var y=th-1;y>=0;y--)
            for(var x=tw-1;x>=0;x--){
                ...
                if(ex>=0 && ey>=0 && ex*tilesize<planewidth && ey*tilesize<planeheight){
                    var key=`${level}.${ex}.${ey}`;//cfg.Key(level,ex,ey);
                    var tile=cache.get(key);
                    if(!tile){
                        loading.push({x:x,y:y,level:level,ex:ex,ey:ey,key:key});
...
//                                level++;
                                level--;
                                key=`${level}.${ex}.${ey}`;//cfg.Key(level,ex,ey);
                                tile=cache.get(key);
                            }
                            if(tile)
        ...
        (function loadloop(){
            ...
            cfg.Load(loaditem.key,current_section,loaditem.level,loaditem.ex,loaditem.ey,function(tile){
